...
// TODO: use better tooltip: https://github.com/dolr-ai/product-roadmap/issues/701
#[component]
pub fn TooltipBottomRight(
    #[prop(into)] icon: icondata_core::Icon,
    #[prop(into)] title: String,
    #[prop(into)] description: String,
) -> impl IntoView {
    ...
    view! {
        <div class="relative group">
            ...
            <div class="absolute top-0 -right-3 z-50 p-4 mt-8 ml-2 w-max rounded-md opacity-0 duration-150 pointer-events-none group-hover:opacity-100 max-w-[65vw] bg-primary-200 text-primary-800 md:max-w-[400px]">
                <div class="absolute right-[15px] bottom-full mr-1 w-0 h-0 border-r-4 border-b-4 border-l-4 border-l-transparent border-r-transparent border-b-primary-200"></div>
                <h2 class="font-bold">{title}</h2>
                <div>{description}</div>
            </div>
        </div>
